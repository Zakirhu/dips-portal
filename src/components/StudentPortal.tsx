...
export const StudentPortal: React.FC<StudentPortalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'study_hub' | 'announcements' | 'profile'>('study_hub');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-950 text-white p-6 rounded-2xl shadow-lg border border-emerald-900/60">
        ...
        <div className="mt-4 pt-4 border-t border-emerald-900/50 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-300">
          ...
          <span className="text-slate-400 text-[11px]">{resources.length} Available Learning Materials</span>
        </div>
      </div>
          ...
          {filteredResources.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredResources.map((res) => (
                <div
                  key={res.id}
                  className="p-5 rounded-xl bg-white border border-slate-200 shadow-xs hover:border-emerald-300 hover:shadow-md transition-all flex flex-col justify-between"
                >
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-xs transition-colors flex items-center justify-center gap-1.5"
                    >
