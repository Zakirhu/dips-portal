...
export const ResourcePreviewModal: React.FC<ResourcePreviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-950/75 backdrop-blur-xs">
      <div className="relative w-full h-full sm:h-auto sm:max-h-[92vh] max-w-4xl flex flex-col bg-white sm:rounded-2xl shadow-2xl border-0 sm:border border-slate-200 overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between p-4 sm:p-5 border-b border-slate-100 bg-slate-50/70 shrink-0">
          <div className="flex items-start gap-3 sm:gap-3.5 pr-2 sm:pr-6">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white p-1 flex items-center justify-center shadow-xs border border-slate-200 shrink-0 mt-0.5">
              <img
                src="/dips-logo.png"
            ...
            <div className="space-y-1 min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                <span className="px-2 py-0.5 text-[10px] sm:text-xs font-semibold rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200">
                  {resource.subjectName}
                </span>
                <span className="px-2 py-0.5 text-[10px] sm:text-xs font-semibold rounded-md bg-amber-50 text-amber-700 border border-amber-200">
                  {resource.className}
                </span>
                <span className="px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-md bg-slate-100 text-slate-700">
                  {resource.category}
                </span>
                <span className="px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-md bg-blue-50 text-blue-700">
                ...
                {resource.status === 'pending_approval' && (
                  <span className="px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-md bg-yellow-100 text-yellow-800 flex items-center gap-1">
                    <ShieldAlert className="w-3 h-3" /> Pending Review
                  </span>
                )}
              </div>
              <h2 className="text-base sm:text-xl font-bold text-slate-900 leading-snug line-clamp-2">{resource.title}</h2>
        ...
        {/* Footer Actions (Responsive for Phone Screens) */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between p-3.5 sm:p-4 border-t border-slate-200 bg-slate-50/90 gap-3 shrink-0">
          <div className="flex items-center justify-between sm:justify-start gap-2 text-xs text-slate-500">
            <span>{resource.viewsCount || 0} views • {resource.downloadsCount || 0} downloads</span>
            {currentUser && (
              <button
                onClick={() => setShowRateModal(true)}
                className="sm:hidden px-2.5 py-1 text-xs font-semibold text-amber-800 bg-amber-50 border border-amber-200 rounded-lg hover:bg-amber-100 transition-colors flex items-center gap-1"
              >
                <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-600" />
                <span>Rate</span>
              </button>
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-2">
            {currentUser && (
              <button
                onClick={() => setShowRateModal(true)}
                className="hidden sm:flex px-3 py-2 text-xs font-semibold text-amber-800 bg-amber-50 border border-amber-200 rounded-xl hover:bg-amber-100 transition-colors items-center gap-1.5 min-h-[40px]"
              >
                <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-600" />
            ...
            {onOpenVersions && (
              <button
                onClick={() => onOpenVersions(resource)}
                className="flex-1 sm:flex-initial px-3 py-2 text-xs font-medium text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors flex items-center justify-center gap-1.5 min-h-[40px]"
              >
                <History className="w-3.5 h-3.5" /> Revisions ({resource.versions?.length || 1})
              </button>
            )}
            ...
            {canCollaborate && onCollaborate && (
              <button
                ...
                className="flex-1 sm:flex-initial px-3 py-2 text-xs font-semibold text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-xl hover:bg-indigo-100 transition-colors min-h-[40px] flex items-center justify-center"
              >
                + Update Version
              </button>
            )}
            ...
            <button
              ...
              className="w-full sm:w-auto px-5 py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md transition-colors flex items-center justify-center gap-2 cursor-pointer min-h-[42px] active:scale-[0.98]"
            >
